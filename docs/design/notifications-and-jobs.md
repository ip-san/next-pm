...
- 期限切れの一時アップロード(添付ファイル)は、**次に誰かが新規アップロードした瞬間についでに掃除される**(`application/attachments/prune-pending-uploads.ts`、`createPendingUpload`の冒頭で毎回呼ばれる)——Redmineの`Attachment.prune`のような専用cron/rakeタスクではなく、このコードベース全体で使われている「次にそのデータに触れた瞬間についでに行う」遅延実行パターンの一例。`redeemUploadToken`側の期限チェック(トークン引き換え自体を拒否)はそのまま残っており、次のスイープが走るまでの間の防御としても機能する。**ただし、新規アップロードが二度と発生しないまま放置されると、掃除のトリガー自体が来ないため対象が残り続ける**——この後述のトレードオフを理解した上での意図的な選択。
- ウォッチのプルーニング、Webhook配信なども同様に**存在しない**(Webhook機能自体がnext-pmに無い)。

この「非同期処理はnotifyジョブ1種類・時刻トリガーは一切無し」という構成そのものは変わっていない——添付ファイルのGCも上記の通り遅延実行で済ませており、`jobs`テーブルに新しい`jobType`を足す形にはしていない。SCM自動フェッチのように「何もトリガーする操作が無くても定期的に実行する必要がある」機能を将来追加する場合は、この構成では実現できない(時刻トリガー自体の仕組みが無い)ことに注意——その場合は別途スケジューラ(cronプロセス、または`pg_cron`等)の導入が必要になる。
